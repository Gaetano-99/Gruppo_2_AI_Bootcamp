...
* **Data & Storage Layer:**
  ...
  * I vettori degli embedding dei chunk semantici risiedono in un vector store esterno (es. ChromaDB), collegato a SQLite tramite `materiali_chunks.id` come `document_id`.
...
**`users`**
Tabella unificata per tutti gli utenti autenticati. Il campo `ruolo` (`'studente'`, `'docente'`, `'admin'`) discrimina il tipo di utente e governa il routing delle pagine Streamlit. I campi specifici per ruolo sono nullable: ogni utente popola solo i propri.
- Campi studente: `data_nascita`, `telefono`, `matricola_studente`, `corso_di_laurea_id`, `anno_corso`
- Campi docente: `matricola_docente`, `dipartimento`
- Campi admin: nessuno aggiuntivo — record inseriti manualmente

> **Vincolo applicativo:** SQLite non supporta CHECK constraint cross-tabella. Il codice in `db_handler.py` deve verificare `users.ruolo` prima di ogni INSERT su tabelle che usano `studente_id` o `docente_id` come FK.

**`corsi_di_laurea`**
Il percorso accademico (es. "Ingegneria Informatica"). Un utente studente appartiene a un solo corso di laurea tramite `users.corso_di_laurea_id`.

**`corsi_universitari`**
La singola materia accademica (es. "Basi di Dati"). Appartiene a uno o più corsi di laurea tramite `corsi_laurea_universitari`. Ha un solo docente titolare (`docente_id → users.id` con `ruolo='docente'`).

**`materiali_didattici` + `materiali_chunks`**
Fonte di verità assoluta per il RAG. I materiali caricati dal docente (PDF, slide, video) vengono segmentati in chunks semantici dal Document Processor. I chunks sono la base esclusiva per la generazione di qualsiasi contenuto AI e non vanno mai svuotati al riavvio. Il campo `embedding_sync` traccia se il chunk è già stato vettorizzato nel vector store esterno — stesso principio di idempotenza di `is_processed` per i materiali grezzi.

**`piani_personalizzati`**
Piano di studio generato dall'AI per lo studente (`studente_id → users.id` con `ruolo='studente'`). Non collegato direttamente ai corsi universitari (eccetto tipo `'esame'`). Struttura gerarchica: `piano_capitoli` → `piano_paragrafi` → `piano_contenuti`.
...
Sistema di valutazione formativa. Tre tipi distinti (A: piano privato studente, B: corso privato studente, C: corso approvato docente). Solo i quiz con `approvato=1` alimentano i report del docente. `studente_id` e `docente_id` puntano entrambi a `users.id` — il nome della colonna porta il significato semantico del ruolo atteso.

**`lezioni_corso`**
Lezioni generate dall'AI o scritte dal docente, associate a un corso universitario. Visibili agli studenti solo dopo approvazione (`approvato=1`). `docente_id → users.id` con `ruolo='docente'`.
...
2. LangGraph attiva il nodo content_gen.py
        ↓
3. rag_engine.py esegue query ibrida:
   a. ChromaDB: similarity_search(argomento, filter={"corso_id": X}) → [chunk_ids]
...
5. Il JSON del quiz viene scritto su:
   - quiz (tipo A o B, approvato=0, studente_id=users.id)
