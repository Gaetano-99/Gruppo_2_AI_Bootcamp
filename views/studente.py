...
def _render_sidebar_piani(studente_id: int):
    ...
    for piano in piani:
        ...
        # ---- Plan Optimizer: editor capitoli (solo per il piano attivo) ----
        if attivo:
            with st.expander("✏️ Gestisci struttura", expanded=False):
                capitoli_edit = db.trova_tutti("piano_capitoli", {"piano_id": pid}, ordine="ordine ASC")
                if not capitoli_edit:
                    st.caption("Nessun capitolo. Chiedi a Lea di generare il contenuto del piano.")
                else:
                    for cap in capitoli_edit:
                        cid = cap["id"]
                        st.markdown(f"**Capitolo {cap['ordine'] + 1}: {cap['titolo']}**")
                        col_rin, col_up, col_dn, col_dc = st.columns([4, 1, 1, 1])
                        with col_rin:
                            nuovo_titolo = st.text_input(
                                "Rinomina", value=cap["titolo"],
                                key=f"_rin_cap_{cid}", label_visibility="collapsed"
                            )
                        with col_up:
                            if st.button("↑", key=f"_up_cap_{cid}", help="Sposta su"):
                                if cap["ordine"] > 0:
                                    db.aggiorna("piano_capitoli", {"id": cid}, {"ordine": cap["ordine"] - 1})
                                    # Scambia con il precedente
                                    prev = [c for c in capitoli_edit if c["ordine"] == cap["ordine"] - 1]
                                    if prev:
                                        db.aggiorna("piano_capitoli", {"id": prev[0]["id"]}, {"ordine": cap["ordine"]})
                                    st.rerun()
                        with col_dn:
                            if st.button("↓", key=f"_dn_cap_{cid}", help="Sposta giù"):
                                if cap["ordine"] < len(capitoli_edit) - 1:
                                    db.aggiorna("piano_capitoli", {"id": cid}, {"ordine": cap["ordine"] + 1})
                                    nxt = [c for c in capitoli_edit if c["ordine"] == cap["ordine"] + 1]
                                    if nxt:
                                        db.aggiorna("piano_capitoli", {"id": nxt[0]["id"]}, {"ordine": cap["ordine"]})
                                    st.rerun()
                        with col_dc:
                            if st.button("🗑", key=f"_del_cap_{cid}", help="Elimina capitolo"):
                                db.esegui("DELETE FROM piano_capitoli WHERE id = ?", [cid])
                                st.rerun()
                        # Applica rinomina se cambiato
                        if nuovo_titolo and nuovo_titolo != cap["titolo"]:
                            if st.button("Salva nome", key=f"_save_cap_{cid}"):
                                db.aggiorna("piano_capitoli", {"id": cid}, {"titolo": nuovo_titolo})
                                st.rerun()
                        st.divider()

                # Aggiungi nuovo capitolo
                nuovo_cap_title = st.text_input("Titolo nuovo capitolo", key=f"_new_cap_{pid}", placeholder="Es. Capitolo aggiuntivo")
                if st.button("+ Aggiungi capitolo", key=f"_add_cap_{pid}", use_container_width=True):
                    if nuovo_cap_title.strip():
                        max_ord = len(capitoli_edit)
                        db.inserisci("piano_capitoli", {
                            "piano_id": pid,
                            "titolo": nuovo_cap_title.strip(),
                            "ordine": max_ord,
                            "completato": 0,
                        })
                        st.rerun()
                    else:
                        st.warning("Inserisci un titolo per il capitolo.")
