...
def _get_tutti_piani_studente(studente_id: int) -> list[dict]:
    ...
    return db.esegui("""
        ...
        JOIN corsi_universitari cu ON cu.id = pp.corso_universitario_id
                ...
                for q_idx, c in enumerate(quiz_list):
                    ...
                    if not domande and c.get("quiz_id"):
                        ...
                        domande = [
                            {
                                "testo": r["testo"],
                                "risposta_corretta": r.get("risposta_corretta") or "",
                    ...
                    for d_idx, d in enumerate(domande[:5], 1):
                        testo_d = (d.get("testo") or d.get("domanda") or d.get("question") or "")
                        testo_d_safe = testo_d.replace("&", "&amp;").replace("<", "&lt;").replace(">", "&gt;")

                        # Risposta e spiegazione
                        risposta = (d.get("risposta_corretta") or d.get("answer") or d.get("risposta") or "")
                        spiegazione = (d.get("spiegazione") or d.get("explanation") or "")

                        # Opzioni (scelta multipla)
                        opzioni = d.get("opzioni") or []
                        opzioni_raw = d.get("opzioni_json")
                        if opzioni_raw and not opzioni:
                            try:
                                opzioni = json.loads(opzioni_raw)
                            except Exception:
                                pass

                        # Risolvi risposta lettera → testo
                        if risposta and len(risposta.strip()) == 1 and risposta.strip().upper() in "ABCD" and opzioni:
                            idx_l = ord(risposta.strip().upper()) - ord("A")
                            risposta_testo = opzioni[idx_l] if 0 <= idx_l < len(opzioni) else risposta
                        else:
                            risposta_testo = risposta

                        opzioni_html = ""
                        if opzioni:
                            lettere = "ABCD"
                            opzioni_html = '<div class="quiz-opzioni">' + "".join(
                                f'<div class="quiz-opzione">{lettere[k] if k < 4 else k + 1}. '
                                f'{opt.replace("<","&lt;").replace(">","&gt;")}</div>'
                                for k, opt in enumerate(opzioni)
                            ) + "</div>"

                        uid = f"qa_{i}_{p_idx}_{q_idx}_{d_idx}"
                        risposta_safe = risposta_testo.replace("&", "&amp;").replace("<", "&lt;").replace(">", "&gt;")
                        spiegazione_safe = spiegazione.replace("&", "&amp;").replace("<", "&lt;").replace(">", "&gt;")

                        # Domanda + opzioni
                        st.markdown(
                            f'<div class="quiz-item">'
                            f'<div class="quiz-domanda">'
                            f'<span class="quiz-num">{d_idx}</span>{testo_d_safe}'
                            f'</div>'
                            f'{opzioni_html}'
                            f'</div>',
                            unsafe_allow_html=True,
                        ...
                        # Reveal risposta con st.button (non usa <input> che Streamlit sanitizza)
                        if risposta_safe:
                            reveal_key = f"_qa_{uid}"
                            if st.session_state.get(reveal_key):
                                answer_html = f"<strong>✅ Risposta:</strong> {risposta_safe}"
                                if spiegazione_safe:
                                    answer_html += f'<br><span style="color:#5A6A7E;font-size:0.79rem">{spiegazione_safe}</span>'
                                st.markdown(
                                    f'<div class="qa-answer" style="display:block;">{answer_html}</div>',
                                    unsafe_allow_html=True,
                                )
                            else:
                                if st.button("👁 Mostra risposta", key=f"qa_show_{uid}"):
...
def _dialog_upload_materiale(corso_id: int, corso_nome: str, studente_id: int):
    st.markdown(f"Carica materiale per **{corso_nome}**")
    uploaded = st.file_uploader(
        "Seleziona un file",
        type=["pdf", "docx", "txt", "pptx", "xlsx"],
        key="upload_file_dialog",
    )
    if uploaded:
        titolo = st.text_input("Titolo", value=uploaded.name)
        tipo = st.selectbox("Tipo", ["pdf", "docx", "txt", "pptx", "altro"],
                            index=["pdf", "docx", "txt", "pptx", "altro"].index(
                                uploaded.name.rsplit(".", 1)[-1].lower()
                                if uploaded.name.rsplit(".", 1)[-1].lower() in ["pdf", "docx", "txt", "pptx"]
                                else "altro"
                            ))
        if st.button("Carica", type="primary", use_container_width=True):
            corso_dir = os.path.join(_UPLOAD_DIR, str(corso_id))
            os.makedirs(corso_dir, exist_ok=True)
            file_path = os.path.join(corso_dir, uploaded.name)
            with open(file_path, "wb") as f:
                f.write(uploaded.getbuffer())
            try:
                db.inserisci("materiali_didattici", {
                    "corso_universitario_id": corso_id,
                    "docente_id": studente_id,
                    "titolo": titolo,
                    "tipo": tipo,
                    "s3_key": f"uploads/{corso_id}/{uploaded.name}",
                    "is_processed": 0,
                })
            except Exception:
                pass
            st.success(f"✅ {uploaded.name} caricato con successo!")
            st.rerun()


@st.dialog("Materiale didattico caricato")
def _dialog_view_materiale(corso_id: int, corso_nome: str):
    ...
    if not materiali:
        st.caption("Nessun materiale caricato per questo corso.")
        return

    for m in materiali:
        with st.container(border=True):
            c1, c2 = st.columns([5, 1])
            c1.markdown(f"**{m['titolo']}**")
            c2.caption(m.get("tipo", "").upper())
            st.caption(
                f"Caricato il: {(m.get('caricato_il') or '')[:10] or '—'}  ·  "
                + ("✅ Elaborato" if m.get("is_processed") else "⏳ In attesa di elaborazione")
            )
    ...
    with col_cx:
        if not view_mode:
            ...
            if piano_docente:
                ...
                _render_contenuto_piano(piano_docente["id"])
                st.markdown("---")
            else:
                st.info(
            ...
            # — Piani personalizzati dello studente per questo corso —
            piani = _get_piani_corso(studente_id, corso_sel_id)
            if piani:
                st.markdown("#### I miei piani di studio")
                for piano in piani:
                    pid = piano["id"]
                    ...
                    with col_a:
                        ...
                        <div class="piano-card">
                            ...
                            <div class="piano-meta">
                                <span class="ai-badge">AI</span>
                                &nbsp;{n_cap} capitoli · {n_par} sezioni
                                &nbsp;·&nbsp;{piano.get("created_at", "")[:10]}
                            </div>
                        </div>
                        """, unsafe_allow_html=True)
                    with col_b:
                        if st.button("Studia", key=f"open_piano_cx_{pid}", use_container_width=True):
                            st.session_state["_piano_sel"] = pid
                            st.session_state["_view_mode"] = "piano"
                            st.rerun()
            else:
                st.caption("Nessun piano ancora per questo corso. Chiedi a Lea di generarne uno!")
            ...
            <div class="section-sub">{corso_sel_nome} · Piano personalizzato</div>
            """, unsafe_allow_html=True)

            col_back, col_del_piano, _ = st.columns([1, 1, 4])
            with col_back:
                if st.button("← Corso"):
                    st.session_state["_view_mode"] = "corso"
                    st.session_state["_piano_sel"] = None
            ...
            # Materiale didattico (caricabile solo nel contesto del piano)
            with st.container(horizontal=True):
                if st.button(":material/upload_file: Carica materiale", key="btn_upload_mat"):
                    _dialog_upload_materiale(corso_sel_id, corso_sel_nome, studente_id)
                if st.button(":material/folder_open: Visualizza materiale", key="btn_view_mat"):
                    _dialog_view_materiale(corso_sel_id, corso_sel_nome)
