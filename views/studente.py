...
def _render_contenuto_piano(piano_id: int, studente_id: int = 0, is_corso_docente: bool = False):
    ...
    <style>
        * {{ margin:0; padding:0; box-sizing:border-box; }}
        ...
        .toc-count {{ margin-left:auto; color:#5A6A7E; font-size:0.75rem; font-weight:400; }}
    </style>
    <div class="nav-toc">
...
def _render_chatbot(
    ...
    sug_cols = st.columns(len(suggerimenti))
    for sug_i, (icona, testo) in enumerate(suggerimenti):
        with sug_cols[sug_i]:
            if st.button(f"{icona} {testo}", key=f"sug_{sug_i}", disabled=is_processing, use_container_width=True):
                if doc_chat:
...
def mostra_homepage_studente():
    ...
    # -------------------------------------------------------------------------
    # RACCOMANDAZIONI — full-width, solo nella home
    # -------------------------------------------------------------------------
    if not view_mode:
